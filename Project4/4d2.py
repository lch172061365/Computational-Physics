...
def flipping(grid, beta):
    ...
    for i in range(N):
        for j in range(N):
                ...
                if cost < 0:
                    s *= -1
                # generate random number from 0 to 1, if P < exp(-E/(kT)), then s flip
...
i = 0
number = 1000 # samples numbers, which is the total number for distribution
while i < number:
    grid = init_state(N)
    ei = calculate_energy(grid)
    Energy.append(ei)
    i += 1
